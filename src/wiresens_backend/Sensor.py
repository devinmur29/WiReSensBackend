...
class Sensor():
    def __init__(self, selWires:int, readWires:int,numNodes, id, deviceName = "Esp1", intermittent = False, p=15, fileName=None, port="COM3",baud="250000", recording_dir="./recordings/"):
        self.id = id
        self.readWires = readWires
        self.selWires = selWires
        self.deviceName = deviceName
        self.path = os.path.join(recording_dir, deviceName + "_" + fileName) if fileName is not None else os.path.join(recording_dir, deviceName + "_sensor_" + str(id) + "_" + str(int(time.time())) + ".hdf5")
